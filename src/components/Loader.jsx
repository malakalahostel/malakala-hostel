...
export default function Loader({ onFinish }) {
  ...
  const [phase, setPhase] = useState(0); // 0=Wait, 1=Spinning, 2=Terminal, 3=Progress, 4=Countdown
  ...
  // Main 60-second Launch Controller
  useEffect(() => {
    if (phase === 1) {
      // Phase 1: Logo spins and disappears (5 seconds)
      ...
      // Total so far: 5 + 20 + 20 + 15 = 60 seconds
  ...
  const handleLaunch = () => {
    setPhase(2); // Skip Phase 1 directly to terminal sequence
  };
    ...
    <AnimatePresence>
      {isVisible && (
        ...
        >
          ...
          <AnimatePresence>
            {phase <= 1 && (
              <motion.div 
                ...
                animate={phase === 1 ? { scale: [1, 1.2, 0], rotateZ: [0, 360, 1080], opacity: [1, 1, 0] } : {}}
                transition={phase === 1 ? { duration: 5, ease: "easeInOut" } : { duration: 0 }}
                exit={{ opacity: 0 }}
              >
                <Canvas camera={{ position: [0, 0, 5], fov: 50 }}>
                  ...
                  <Float speed={phase === 1 ? 25 : 4} rotationIntensity={phase === 1 ? 10 : 1} floatIntensity={phase === 1 ? 0 : 2}>
                    <Center>
                      <Text3D
          ...
          {/* Phase 1: Warning Text has been removed as per user request */}
